...
\end{enumerate} 
\textbf{Solution:} Let, $X_i$ be the random variable that represent the a single toss resulting in head and $Y$ be the random variable that represent the total tosses.
\begin{table}[h]
	\centering
	\input{tables/table} 
	\caption{} 
...
The pmf of $Y$ is,
\begin{align}
\label{eq:pr-pts/1}\pr{Y = k}=\comb{n}{k}\brak{p}^k\brak{q}^{n-k} && ,1 \leq k \leq n
\end{align}
